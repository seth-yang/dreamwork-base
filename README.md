...
### org.dreamwork.concurrent.broadcast
提供本地广播机制。

初始化广播组件环境
```java
public class GlobalContext {
    public static final ILocalBroadcastService broadcaster;
...
```java
final ILocalBroadcastReceiver receiver = (category, message) -> {
    System.out.println ("received message: " + message + " from category: " + category);
};
GlobalContext.broadcaster.register ("some.category", receiver);
```

发出广播
```java
final int ACTION_CREATE_DATE = 0;
GlobalContext.broadcaster.broadcast ("some.category", ACTION_CREATE_DATE, new Date ());
```
